...
use ::proto_server::errors::{ResultExt, Result};

pub struct ProtoServer;

impl ProtoServer {
    pub fn run_server(port: u16, db_server: Arc<TreeServer>) -> Result<JoinHandle<Result<()>>> {
        Ok(thread::spawn(move || { Self::_start(port, db_server) }))
    }

    fn _start(port: u16, db_server: Arc<TreeServer>) -> Result<()> {
        let env = Rc::new(RefCell::new(Environment::new(db_server)));
...
impl<'e> Environment<'e> {
    fn new(db_server: Arc<TreeServer>) -> Self {
        Environment {
            db_server,
            watches: Watches::new(),
